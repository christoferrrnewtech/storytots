...
import 'package:storytots/data/local/app_database.dart';
import 'package:storytots/data/local/session_service.dart';
...
/// Aggregates reading stats fully offline: minutes from the SharedPreferences
/// daily aggregates, completed stories + sentences practiced from SQLite.
class ProfileStatsService {
  ...
  Future<ProfileStats> getStats() async {
    ...
    // Sentences practiced: sum of (max sentence_index + 1) per story for the
    // current user, read from the SQLite reading_progress table.
    int sentencesPracticed = 0;
    final uid = SessionService.instance.currentUserId;
    if (uid != null) {
      final db = await AppDatabase.instance.db;
      final rows = await db.rawQuery(
        'SELECT story_id, MAX(sentence_index) AS max_idx '
        'FROM reading_progress WHERE user_id = ? GROUP BY story_id',
        [uid],
      );
      for (final r in rows) {
        final maxIdx = (r['max_idx'] as num?)?.toInt() ?? -1;
        sentencesPracticed += (maxIdx + 1);
      }
    }
  ...
  /// Offline alias kept for call-site compatibility (there is no DB-vs-server
  /// distinction anymore — everything is local).
  Future<ProfileStats> getStatsDbFirst() => getStats();
}
